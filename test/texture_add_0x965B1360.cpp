...
int main(int argc, char **argv) {
  ...
  if (!InjectTextureSampleIntoEntryPoint(*shader.module, *shader.dxilModule,
                                         textureDesc)) {
  ...
  if (CountDxOpCalls(*entryFunction, "dx.op.sample.f32") !=
    ...
    std::cerr
        << "Expected one additional sample.f32 call after texture injection.\n";
    return 1;
  }
  ...
  if (addedSrvElementType == nullptr || !addedSrvElementType->isStructTy() ||
    ...
    std::cerr
        << "Added SRV did not use the canonical Texture2D<float4> type.\n";
    return 1;
  }
  ...
  if (reloadedSrv->GetLowerBound() != textureDesc.binding.GetBindPoint() ||
      ...
      reloadedSrv->IsRW()) {
    std::cerr
        << "Reloaded SRV metadata did not match the injected Texture2D SRV.\n";
    return 1;
  }
  ...
  if (CountDxOpCalls(*patchedEntryFunction, "dx.op.sample.f32") !=
      initialSampleCount + 1) {
    std::cerr
        << "Reloaded shader did not preserve the injected sample.f32 call.\n";
    return 1;
  }
