...
int main(int argc, char **argv) {
  if (argc != 3) {
    std::cerr << "Usage: declarative_ign_emit_recipe_0x56C468C3 <input.cso> "
                 "<recipe.yml>\n";
    return 1;
  }
  ...
  if (recipeContext.totalRuleMatches == 0) {
    std::cerr
        << "Expected declarative IGN emit recipe to apply at least one rule.\n";
    return 1;
  }
  ...
  if (finalIgnCount >= initialIgnCount) {
    std::cerr << "Expected declarative IGN emit rewrite to reduce IGN chain "
                 "count from "
  ...
  if (finalTextureLoadCount <= initialTextureLoadCount) {
    std::cerr << "Expected declarative IGN emit rewrite to increase "
