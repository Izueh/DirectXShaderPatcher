...
int main(int argc, char **argv) {
  if (argc != 3) {
    std::cerr << "Usage: declarative_ign_emit_inmemory_recipe_0x56C468C3 "
  ...
  if (!ExecuteDxilRecipe(parseResult.recipe, *shader.module, *shader.dxilModule,
                         &recipeContext, false)) {
  ...
  if (recipeContext.totalRuleMatches == 0) {
    std::cerr << "Expected in-memory declarative IGN emit recipe to apply at "
                 "least one rule.\n";
    return 1;
  }
  ...
  if (finalIgnCount >= initialIgnCount) {
    std::cerr << "Expected in-memory declarative IGN emit rewrite to reduce "
                 "IGN chain count from "
              << initialIgnCount << ", but saw " << finalIgnCount << ".\n";
    return 1;
  }

  if (finalTextureLoadCount <= initialTextureLoadCount) {
    std::cerr << "Expected in-memory declarative IGN emit rewrite to increase "
                 "textureLoad.f32 count from "
              << initialTextureLoadCount << ", but saw "
              << finalTextureLoadCount << ".\n";
    return 1;
  }

  std::cout
      << "In-memory declarative IGN emit rewrite reduced IGN chain count from "
      << initialIgnCount << " to " << finalIgnCount
      << " and increased textureLoad.f32 count from " << initialTextureLoadCount
      << " to " << finalTextureLoadCount << ".\n";
  std::cout.flush();
  std::cerr.flush();
}