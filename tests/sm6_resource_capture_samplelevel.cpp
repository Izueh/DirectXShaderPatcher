...
int main() {
  ...
  const auto recipe_path = RepoRootPath() / "tests/recipes/sm6_resource_capture_samplelevel.recipe.yml";
  ...
  auto it = result->resource_usage.find("sample_levels");
  if (it == result->resource_usage.end()) {
    std::cerr << "Expected 'sample_levels' key in resource_usage map\n";
    return 1;
  }

  std::cout << "SM6 resource capture SampleLevel test passed\n";
  std::cout << "  Match count: " << apply_res->match_count << "\n";
  std::cout << "  sample_levels exports: " << 1 << "\n";
  return 0;
}
