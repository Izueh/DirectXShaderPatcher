...
int main() {
  ...
  const auto recipe_path = RepoRootPath() / "tests/recipes/sm6_resource_capture_textureload.recipe.yml";
  ...
  auto it = result->resource_usage.find("texture_loads");
  if (it == result->resource_usage.end()) {
    std::cerr << "Expected 'texture_loads' key in resource_usage map\n";
    return 1;
  }

  std::cout << "SM6 resource capture TextureLoad test passed\n";
  std::cout << "  Match count: " << apply_res->match_count << "\n";
  std::cout << "  texture_loads exports: " << 1 << "\n";
  return 0;
}
