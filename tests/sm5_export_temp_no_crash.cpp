...
int main() {
  ...
  const auto recipe_path = RepoRootPath() / "tests/recipes/sm5_export_temp_no_crash.recipe.yml";
  ...
  // Verify that temp exports are not added (temp is not a resource/immediate type)
  auto it = result->resource_usage.find("temps");
  if (it != result->resource_usage.end()) {
    std::cerr << "Expected no 'temps' key in resource_usage map (temp is not exportable)\n";
    return 1;
  }

  std::cout << "SM5 export temp no crash test passed\n";
  std::cout << "  Match count: " << apply_res->match_count << "\n";
  std::cout << "  temps exports: not present (expected - temp is not a resource type)\n";
  return 0;
}
