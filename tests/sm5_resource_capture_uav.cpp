...
int main() {
  ...
  const auto recipe_path = RepoRootPath() / "tests/recipes/sm5_resource_capture_uav.recipe.yml";
  ...
  auto it = result->resource_usage.find("uav_access");
  if (it == result->resource_usage.end()) {
    std::cerr << "Expected 'uav_access' key in resource_usage map\n";
    return 1;
  }

  std::cout << "SM5 resource capture UAV test passed\n";
  std::cout << "  Match count: " << apply_res->match_count << "\n";
  std::cout << "  uav_access exports: " << 1 << "\n";
  return 0;
}
