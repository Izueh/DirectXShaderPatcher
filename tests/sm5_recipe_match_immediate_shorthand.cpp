...
int main(int argc, char** argv_) {
  ...
  if (argc != 2) {
    std::cerr << "Usage: sm5_recipe_match_immediate_shorthand <input.ps_5_0.cso>\n";
    return 1;
  }
...
steps:
  ...
  auto parse_result = dxp::sm5::Recipe::ParseFromText(recipe_text, "inline-sm5-match-immediate-shorthand");
  if (!parse_result) {
  ...
  std::cout << "SM5 match immediate shorthand test passed.\n";
  return 0;
}
