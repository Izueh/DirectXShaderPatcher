...
// ---------------------------------------------------------------------------
// Opcode name table – mirrors D3D10_SB_OPCODE_TYPE enum values
// ---------------------------------------------------------------------------
...
uint32_t Program::GetInstructionCount() const {
  return static_cast<uint32_t>(Instructions.size());
}

const ResourceDecl *Program::FindResource(uint32_t bindPoint) const {
  for (const auto &decl : Resources) {
    if (decl.RegisterBindPoint == bindPoint)
      return &decl;
  }
  return nullptr;
}

const CBufferDecl *Program::FindCBuffer(uint32_t bindPoint) const {
  for (const auto &decl : CBuffers) {
    if (decl.RegisterBindPoint == bindPoint)
      return &decl;
  }
  return nullptr;
...
const SamplerDecl *Program::FindSampler(uint32_t bindPoint) const {
  for (const auto &decl : Samplers) {
    if (decl.RegisterBindPoint == bindPoint)
      return &decl;
  }
  return nullptr;
